...
export const loadWineList = createAsyncThunk(
  ...
  async (params: MergedParams, { rejectWithValue }) => {
    try {
      let wineList: PageableResponse<WineType>;

      if (params.name.length > 0) {
        wineList = await searchWinesService(params.name, params.filters.page);
      } else {
        wineList = await getWinesService(params.filters);
      }
